...
// this function requires an m3u8 video stream and the number of seconds to transcribe, i.e.
...
func M3U8VideoStreamToFile(m3u8VideoStream string, fileName string, fileType string, secondsString string) error {
	// write to flac file
	cmd := exec.Command("ffmpeg", "-y", "-i", m3u8VideoStream, "-t", secondsString, "-f", fileType, "-ar", fmt.Sprint(sampleRateHertz), fileName)
	err := cmd.Start()
	if err != nil {
		fmt.Println(err)
		return err
