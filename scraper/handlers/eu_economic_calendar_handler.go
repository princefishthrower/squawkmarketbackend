...
func EUEconomicCalendarHandler(squawks *[]string, url string) func(e *colly.HTMLElement) {
	return func(e *colly.HTMLElement) {
		...
		if len(*squawks) == 0 && strings.Contains(tds.Eq(1).Text(), "EUR") && utils.NotEmpty(tds.Eq(4).Text()) {
			// build the squawk phrase
			squawk := "Eurozone " + tds.Eq(3).Text() + " is " + tds.Eq(4).Text()
